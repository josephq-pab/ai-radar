...
class CircuitLevel(str, Enum):
    """Ralph 断路器三级：CRITICAL 立即中断 / BREAK 告警继续 / WARN 仅警告"""
    CRITICAL = "CRITICAL"   # 立即中断 pipeline
    BREAK = "BREAK"         # 告警但不退出
    WARN = "WARN"           # 仅警告，不阻断
...
# 每个脚本的断路器级别（按脚本名匹配）
CIRCUIT_EXIT_CODES = {
    'parse_initial_data.py': CircuitLevel.CRITICAL,   # 根数据，失败必须停
    'fetch_analyst_articles.py': CircuitLevel.WARN,    # 可选，失败仅警告
    'build_analyst_opinions.py': CircuitLevel.WARN,   # 可选，失败仅警告
    'generate_weekly_report.py': CircuitLevel.CRITICAL,  # 核心产物，失败必须停
    'generate_review_queue.py': CircuitLevel.BREAK,     # 中间层，失败应告警
    'build_review_status.py': CircuitLevel.BREAK,       # 中间层，失败应告警
    'build_tracking_items.py': CircuitLevel.BREAK,      # 中间层，失败应告警
    'build_web_bundle.py': CircuitLevel.BREAK,          # bundle 失败要告警
}


def run_step_circuit(name: str, cmd: list[str], timeout: int = 60) -> int:
    """
    Ralph 断路器版 step runner

    - CRITICAL: 失败直接 sys.exit(1)
    - BREAK:    失败打印断路告警，但不退出 pipeline
    - WARN:     失败打印警告，但不退出
    """
    print(f'\n  ▶ {name}')
    print(f'    {" ".join(str(c) for c in cmd)}')
    ...
    if result.returncode != 0:
        ...
        level = CIRCUIT_EXIT_CODES.get(script_name, CircuitLevel.BREAK)

        if level == CircuitLevel.CRITICAL:
            print(f'    🔴 [CIRCUIT_CRITICAL] {name} 失败，根数据错误，立即中断 pipeline')
            if result.stderr:
                for line in result.stderr.strip().split('\n')[:3]:
                    print(f'       └ [ERR] {line}')
            sys.exit(1)

        elif level == CircuitLevel.BREAK:
            print(f'    🟡 [CIRCUIT_BREAK] {name} 失败，中间层数据可能不完整，继续执行')
            if result.stderr:
                for line in result.stderr.strip().split('\n')[:3]:
                    print(f'       └ [ERR] {line}')

        else:  # WARN
            print(f'    ⚠️  [CIRCUIT_WARN] {name} 失败，但为辅助数据，继续执行')
            if result.stderr:
                for line in result.stderr.strip().split('\n')[:2]:
                    print(f'       └ [ERR] {line}')
    else:
...
def main() -> None:
    ...
    if mode in ('full', 'parse', 'data'):
        header('Step 1: 数据解析')
        code = run_step_circuit('parse_initial_data.py', [sys.executable, str(SCRIPTS / 'parse_initial_data.py')], timeout=60)
        if code != 0:
            print('\n  ❌ parse 失败，退出。')
            sys.exit(1)
    ...
    if mode in ('full', 'data'):
        ...
        # analyst 抓取可选，失败不中止主链路（断路器级别=WARN）
        code = run_step_circuit('fetch_analyst_articles.py', [sys.executable, str(SCRIPTS / 'fetch_analyst_articles.py')], timeout=120)
        if code != 0:
            print('\n  ⚠️  analyst 抓取失败，继续主链路')
        code2 = run_step_circuit('build_analyst_opinions.py', [sys.executable, str(SCRIPTS / 'build_analyst_opinions.py')], timeout=60)
        if code2 != 0:
    ...
    if mode in ('full', 'track', 'rebuild'):
        ...
        code = run_step_circuit('build_review_status.py', [sys.executable, str(SCRIPTS / 'build_review_status.py')], timeout=30)
        code2 = run_step_circuit('build_tracking_items.py', [sys.executable, str(SCRIPTS / 'build_tracking_items.py')], timeout=30)
    ...
    if mode in ('full', 'report', 'rebuild'):
        header('Step 2: 周报生成')
        code = run_step_circuit('generate_weekly_report.py', [sys.executable, str(SCRIPTS / 'generate_weekly_report.py')], timeout=60)
        if code != 0:
            print('\n  ❌ 周报生成失败，退出。')
            sys.exit(1)

        code = run_step_circuit('generate_review_queue.py', [sys.executable, str(SCRIPTS / 'generate_review_queue.py')], timeout=30)
        if code != 0:
    ...
    if mode in ('full', 'rebuild'):
        ...
        code = run_step_circuit('build_web_bundle.py', [sys.executable, str(SCRIPTS / 'build_web_bundle.py')], timeout=30)
        if code != 0:
            print('\n  ❌ bundle 生成失败，退出。')
            sys.exit(1)
