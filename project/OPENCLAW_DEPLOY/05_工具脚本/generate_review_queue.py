...
def build_ralph_meta(items: list[dict], report_text: str) -> dict:
    """
    生成 _ralphMeta 块（Ralph 双条件退出机制信号）

    条件A: exitSignal = True  ← 所有 items 均来自真实提取（非全兜底）
    条件B: sourceConfirmed = True  ← fallbackRate < 1.0（有至少1条真数据）

    两个同时满足 → 双条件退出达成，视为真完成
    """
    fallback_count = sum(1 for item in items if item.get('text', '') in KNOWN_FALLBACK_TEXTS)
    total_count = len(items)
    fallback_rate = fallback_count / total_count if total_count > 0 else 1.0

    # 条件B：只要有1条真数据即为 sourceConfirmed
    source_confirmed = fallback_count < total_count

    # 条件A：completionMarkers 非空（有完成标记）
    if source_confirmed:
        completion_markers = ['review_queue_generated', 'no_empty_required_fields']
        if fallback_count == 0:
            completion_markers.append('zero_fallback')
        else:
            completion_markers.append(f'partial_fallback_{fallback_count}')
    else:
        completion_markers = []  # 全兜底，无任何完成标记

    # 双条件同时满足才为 True
    both_conditions_met = source_confirmed and len(completion_markers) > 0

    return {
        'exitSignal': both_conditions_met,
...
def main() -> None:
    ...
    # ── Ralph 双条件退出信号 ───────────────────────────
    ralph_meta = build_ralph_meta(items, text)

    payload = {
        'reportFile': str(report_path),
        'reportPreview': text[:4000],
        'items': items,
        '_ralphMeta': ralph_meta,   # Ralph Exit Signal（双条件退出标记）
    }
