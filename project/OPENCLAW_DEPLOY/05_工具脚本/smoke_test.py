...
# ── Ralph 双条件退出检查 ─────────────────────────────────
def check_exit_signal() -> dict:
    """
    Ralph 双条件退出检查（Phase F 新增）

    退出条件（必须同时满足）：
      条件A: _ralphMeta.exitSignal == True
      条件B: _ralphMeta.sourceConfirmed == True

    满足 → PASS（可视为真完成）
    不满足 → FAIL（视为兜底完成，阻断主链路）
    """
    outcomes = {'PASS': 0, 'WARN': 0, 'FAIL': 0}
    print('\n── Ralph Exit Signal 双条件退出检查 ──')
    ...
    meta = rq_data.get('_ralphMeta', {})
    if not meta:
        result('_ralphMeta', 'FAIL',
               'review-queue.json 缺少 _ralphMeta 块（可能为旧版本生成，建议重新运行 generate_review_queue.py）')
        outcomes['FAIL'] += 1
        return outcomes

    exit_signal = meta.get('exitSignal', False)
    source_confirmed = meta.get('sourceConfirmed', False)
    fallback_rate = meta.get('fallbackRate', 1.0)
    fallback_count = meta.get('fallbackCount', '?')
    total_count = meta.get('totalCount', '?')
    markers = meta.get('completionMarkers', [])
    completed_at = meta.get('completedAt', '?')

    # 条件A检查
    result('条件A: exitSignal', 'PASS' if exit_signal else 'FAIL',
           f'exitSignal={exit_signal}')
    outcomes['PASS' if exit_signal else 'FAIL'] += 1

    # 条件B检查
    result('条件B: sourceConfirmed', 'PASS' if source_confirmed else 'FAIL',
           f'sourceConfirmed={source_confirmed}')
    outcomes['PASS' if source_confirmed else 'FAIL'] += 1

    # 兜底率（信息补充）
    result('兜底率', 'PASS' if fallback_rate < 1.0 else 'FAIL',
           f'fallbackRate={fallback_rate:.0%}（{fallback_count}/{total_count} 条）')
    ...
    # 完成标记
    if markers:
        result('completionMarkers', 'PASS', f'{len(markers)} 个标记: {markers}')
        outcomes['PASS'] += 1
    else:
        result('completionMarkers', 'FAIL', '无完成标记（全兜底状态）')
        outcomes['FAIL'] += 1

    # 综合判定
    if exit_signal and source_confirmed:
        result('综合判定', 'PASS',
               f'双条件同时满足，生成时间={completed_at}')
        outcomes['PASS'] += 1
    else:
        result('综合判定', 'FAIL',
               f'双条件未同时满足（exitSignal={exit_signal}, sourceConfirmed={source_confirmed}）'
               f'，本次不应视为真完成，建议人工介入')
        outcomes['FAIL'] += 1
...
def run_all() -> dict:
    ...
    for fn in [validate_parse, validate_review_queue, validate_tracking_items,
               ...
               check_exit_signal,   # Phase F: Ralph 双条件退出检查
